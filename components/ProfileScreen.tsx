  ...
  async function handleToggleNotifications(value: boolean) {
    if (value) {
      ...
      if (!success) {
        Alert.alert(
          'Permission needed',
          'Allow murmur to send notifications in your device settings.',
          [{ text: 'OK' }]
        );
      }
    } else {
      ...
      <Animated.View style={{ opacity: fadeAnim }}>
        ...
        <View style={[styles.tagList, { borderBottomColor: colors.border }]}>
          ...
          <View style={styles.tagRow}>
            ...
            <View style={[styles.tagBarBg, { backgroundColor: colors.surface2, borderColor: colors.border }]}>
              <View style={[styles.tagBar, { backgroundColor: colors.border2, width: `${Math.max(4, (thoughts.filter((t) => !t.tag).length / Math.max(1, user.thoughtsTotal)) * 100)}%` }]} />
            </View>
        ...
        <View style={[styles.reminderSection, { borderBottomColor: colors.border }]}>
          <View style={styles.reminderRow}>
            <View style={styles.reminderLeft}>
              <Text style={[styles.reminderLabel, { color: colors.bright }]}>Daily reminder</Text>
              <Text style={[styles.reminderSub, { color: colors.muted }]}>
                ...
                <View style={styles.timePicker}>
                    ...
                    {HOURS.map((h) => (
                      <TouchableOpacity
                        ...
                        style={[
                          ...
                          pendingHour === h && { borderColor: colors.accent, backgroundColor: colors.accentL },
                        ]}
                        onPress={() => setPendingHour(h)}
                        activeOpacity={0.7}
                      >
                        <Text style={[styles.timeChipText, { color: pendingHour === h ? colors.accent : colors.muted }]}>
                          {pad(h)}
                        </Text>
                      </TouchableOpacity>
                    ))}
                    ...
                    {MINUTES.map((m) => (
                      <TouchableOpacity
                        ...
                        style={[
                          styles.timeChip,
                          { borderColor: colors.border },
                          pendingMinute === m && { borderColor: colors.accent, backgroundColor: colors.accentL },
                        ]}
                        onPress={() => setPendingMinute(m)}
                        activeOpacity={0.7}
                      >
                        <Text style={[styles.timeChipText, { color: pendingMinute === m ? colors.accent : colors.muted }]}>
                          :{pad(m)}
                        </Text>
                      </TouchableOpacity>
                    ))}
                  ...
                  <View style={styles.timeActions}>
                    <TouchableOpacity onPress={() => setEditingTime(false)} activeOpacity={0.7}>
                      <Text style={[styles.timeCancelText, { color: colors.muted }]}>Cancel</Text>
                    </TouchableOpacity>
                    <TouchableOpacity
        ...
        <View style={[styles.appearanceRow, { borderBottomColor: colors.border }]}>
          <View style={styles.appearanceLeft}>
            ...
            <Text style={[styles.appearanceSub, { color: colors.muted }]}>{themes[themeKey].label}</Text>
          </View>
          <TouchableOpacity
        ...
        >
          <View style={[styles.themeModalHeader, { borderBottomColor: colors.border }]}>
            ...
            <TouchableOpacity onPress={() => setThemeModalVisible(false)} activeOpacity={0.7}>
              <Text style={[styles.themeModalClose, { color: colors.muted }]}>Close</Text>
            </TouchableOpacity>
          </View>
          ...
          {(Object.keys(themes) as ThemeKey[]).map((key) => {
            ...
            return (
              <TouchableOpacity
                ...
                style={[
                  styles.themeRow,
                  { backgroundColor: t.colors.bg, borderBottomColor: t.colors.border },
                ...
                activeOpacity={0.85}
              >
                {/* Four swatches showing the palette */}
                <View style={styles.themeRowSwatches}>
                  <View style={[styles.themeRowSwatch, { backgroundColor: t.colors.bg, borderColor: t.colors.border, borderWidth: 1 }]} />
                ...
                {/* Radio dot */}
                <View style={[styles.themeRowRadio, { borderColor: isActive ? t.colors.accent : t.colors.border2 }]}>
...
const styles = StyleSheet.create({
  ...
  themeModalHeader: {
    ...
    padding: 28, borderBottomWidth: 1,
  },
